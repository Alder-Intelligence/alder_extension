...
/*
 * Append a list of String to a StringInfo, separated by commas.
 */
static void
appendStrings(StringInfo buf, List *strs)
{
	ListCell   *lc;
	bool		first = true;
	...
	foreach(lc, strs)
	{
		...
		if (!first)
			appendStringInfoChar(buf, ',');
		appendStringInfo(buf, "%s", lfirst(lc));
		first = false;
	}
}
 ...
 */
static List *
pg_rewrite_query_ex(Query *query)
{
    List       *querytree_list;
...
gp_dump_query_oids(PG_FUNCTION_ARGS)
{
	...
	foreach(lc, raw_parsetree_list)
	{
		...
		queryTree_sublist = pg_rewrite_query_ex(query);
    ...
    elog(NOTICE, "begin collect names");
    foreach(lc, relationOids)
    {
		Oid			relid = lfirst_oid(lc);
        char        *relname = get_rel_name(relid);
        char        *schema = get_namespace_name(get_rel_namespace(relid));
        char        *tablename = palloc(strlen(relname) + strlen(".") + strlen(schema) + 1);
        strcpy(tablename, schema);
        strcat(tablename, ".");
        strcat(tablename, relname);

        relationNames = lappend(relationNames, tablename); 
    }
    elog(NOTICE, "finish collect names");
	...
	appendStringInfoString(&str, "\", \"relnames\": \"");
	appendStrings(&str, relationNames);
